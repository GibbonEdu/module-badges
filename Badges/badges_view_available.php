...
if (isActionAccessible($guid, $connection2, '/modules/Badges/badges_view_available.php') == false) {
    ...
    try {
        $data = array();
        $sql = 'SELECT badgesBadge.name AS award, badgesBadge.logo AS logo, badgesBadge.category AS category FROM badgesBadge WHERE active=\'Y\' ORDER BY category, name';
